...
Program.prototype.compile = function(callback){
  ...
  //spawn new process
  var compile =  exec(lang.compile + lang.fileName+lang.compileExt + " &> compileout.txt > compileout.txt",function(err,stdout,stderr){
    if(err){
      fs.writeFile("compileout.txt",stderr,function(err){
...
/*
  ...
  compile.on('close', (code) => {
    ...
    if(compileErr){
      ...
      fs.writeFile("compileout.txt",compileErr);
      return callback(compileErr);
    }
    else{
      process.chdir(topDir);
